...
DeviceFamily default_log = {
        .Architecture = "Unknown",
        .User         = "User",
        .Password     = "Unknown",
        .DeviceName   = "Unknown MCU",
        .OS           = "Unknown",
        .Device       = "Unknown",
        .Version      = "Unknown"
};

extern EFP efp;  // 定义环境函数指针结构体实例
...
void BIE_UART(void *Parameters, Bie_ShellTypeDef *ShellTypeStruct, EnvVar *env, DeviceFamily *log) {
    printf(RESET_ALL);
    ShellTypeStruct->c = *(uint8_t*)Parameters;  // 从串口读取一个字符
    // 如果是回车键
...
void Env_Task_Switch(EnvVar *userEnv) {
    ...
    for (i = 0; userEnv[i].name != NULL; i++) {
        if(userEnv[i].RunStae){
            // 执行命令
            efp.envpfunc = userEnv[i].callback;  // 设置系统函数指针
            efp.argc = userEnv[i].argc;  // 设置参数个数
            efp.Parameters = userEnv[i].arg;  // 设置参数
...
void Shell_Deal(Bie_ShellTypeDef *ShellTypeStruct, EnvVar *env_vars) {
    ...
    char *args[MAX_ARGS];
    int arg_count = 0;
    static void *arg_part = NULL;  // 参数部分指针
    ...
    arg_part = (arg_count > 1) ? (void *)&args[1] : NULL;  // 提取参数部分
    // 遍历命令列表，匹配命令
    for (int i = 0; i < 20; i++) {
    ...
    for (int i = 0; env_vars[i].name != NULL; i++) {
        if (strcmp(cmd_part, env_vars[i].name) == 0) {
            ...
            Env_Task_Switch(env_vars);  // 执行任务切换
            return;
        }
    }
    // 未匹配到命令
