...
#include <cmath>
#ifdef __OLED_
Dirver::Soft_IIC i2c_Dev(GPIOB, GPIO_Pin_8,GPIO_Pin_9);
...
void OLED_ShowSignedNum(int16_t X, int16_t Y, int32_t Number, uint8_t Length, uint8_t FontSize) {
    ...
    for (i = 0; i < Length; i++)  // 遍历数字的每一位
    {
        ...
        /*+ '0' 可将数字转换为字符格式*/
        OLED_ShowChar(X + (i + 1) * FontSize, Y, Number1 / OLED_Pow(10, Length - i - 1) % 10 + '0',
                      FontSize);
    }
}
...
void OLED_ShowBinNum(int16_t X, int16_t Y, uint32_t Number, uint8_t Length, uint8_t FontSize) {
    ...
    for (i = 0; i < Length; i++)  // 遍历数字的每一位
    {
        ...
        /*+ '0' 可将数字转换为字符格式*/
        OLED_ShowChar(X + i * FontSize, Y, Number / OLED_Pow(2, Length - i - 1) % 2 + '0',
                      FontSize);
    }
}
